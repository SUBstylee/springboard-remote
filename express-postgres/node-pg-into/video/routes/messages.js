/** Routes for messages of pg-intro-demo. */
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        if (results.rows.length === 0) throw new ExpressError(`Can't find message with id of ${req.params.id}`, 404);
...
router.patch('/:id', async (req, res, next) => {
    try {
        ...
        if (result.rows.length === 0) throw new ExpressError(`Can't find message with id of ${req.params.id}`, 404);
