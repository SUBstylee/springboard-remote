...
$pokeButton.click(() => getPokemon());

function getPokemon() {
    $cardContainer.empty();
    let pokeArr = [];
    $.getJSON(`${apiUrl}pokemon`)
        .then(data => {
            // console.log(data);
            for (let i = 0; i < 3; i++) {
                pokeArr.push(Math.floor(Math.random() * data.count));
            };
            return Promise.all(pokeArr.map(num => $.getJSON(`${apiUrl}pokemon/${num}`)));
        })
        .then(pokemonData => {
            // console.log(pokemonData)
            nameSprite = pokemonData.map(p => (
                {
                    name: p.name,
            ...
            return Promise.all(pokeArr.map(num => $.getJSON(`${apiUrl}pokemon-species/${num}`)));
        })
        .then(speciesData => {
